...
export default function RefrescosMenuPage() {
  ...
  useEffect(() => {
    const fetchRefrescos = async () => {
      ...
      if (!error && data) setRefrescos(data);
    };
    fetchRefrescos();
  }, []);
  ...
  const handleAgregar = (item) => {
    ...
    if (cantidad === 0) {
      showNotification("Selecciona al menos 1 refresco 🧋", "error");
      return;
    }
    ...
    const existente = pedidoActual.items.find(
      (i) => i.id === item.id && i.categoria === "refrescos"
    );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-cyan-50 to-sky-100">
        ...
        {notificaciones.map((n) => (
          <div
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg shadow text-white
              ${n.tipo === "error" ? "bg-red-500" : "bg-cyan-600"}
            `}
          >
            {n.tipo === "error" ? (
              <XCircle size={18} />
      ...
      <div className="px-6 pt-4 flex justify-between items-center">
        <div>
          <h1 className="text-2xl font-semibold text-gray-800">🧋 Refrescos</h1>
