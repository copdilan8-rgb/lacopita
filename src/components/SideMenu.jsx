...
let openMenuFn = null; // 👉 variable global temporal

export function openSideMenu() {
  if (openMenuFn) openMenuFn(true);
}

export default function SideMenu() {
  const [open, setOpen] = useState(false);

  // Guardar referencia para poder abrir desde fuera
  useEffect(() => {
    openMenuFn = setOpen;
    return () => {
      openMenuFn = null;
    ...
    <>
      ...
      {open && (
        <div
          ...
          className="fixed inset-0 z-40 backdrop-blur-sm bg-black/30 transition"
        />
      )}
